        ...
        {allImages.length > 1 && (
          <>
            <button
              ...
              className="absolute left-3 top-1/2 -translate-y-1/2 z-10 w-10 h-10 flex items-center justify-center rounded-full bg-white/80 backdrop-blur-sm shadow-md text-[#1b0d11] border border-gray-200 opacity-0 group-hover:opacity-100 transition-all duration-200 hover:bg-[#13ec5b] hover:text-[#0d1b12] hover:border-[#13ec5b] hover:scale-105 disabled:opacity-30 disabled:cursor-not-allowed disabled:hover:scale-100 disabled:hover:bg-white disabled:hover:text-[#1b0d11]"
            >
          ...
          {allImages.map((img, idx) => (
            <button
              ...
              className={`relative aspect-square rounded-xl overflow-hidden border-2 bg-white transition-all ${
                activeImage === idx
                  ? "border-[#13ec5b] shadow-[0_4px_12px_rgba(19,236,91,0.2)] scale-[0.98]"
                  : "border-gray-200 opacity-60 hover:opacity-100 hover:border-[#13ec5b]/50"
              }`}
            >
              <OptimizedImage
                src={img.url}
