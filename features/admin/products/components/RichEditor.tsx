...
/**
 * RichEditor — WYSIWYG editor với 2 tab:
 *   • "visual" : contentEditable WYSIWYG (toolbar formatting)
 *   • "html"   : textarea xem/chỉnh HTML source
 *
 * Chuyển đổi:
 *   visual → html : lấy innerHTML của contentEditable
 *   html → visual : set innerHTML của contentEditable từ nội dung textarea
 *
 * API (ref):
 *   getHTML()         → HTML string hiện tại
 *   isEmpty()         → true nếu không có text nào
 *   setHTML(html)     → set nội dung từ ngoài
 */
...
const RichEditor = (
  ...
  // HTML draft được dùng khi ở tab html
  ...
  useImperativeHandle(ref, () => ({
    getHTML: () => (tab === "html" ? htmlDraft : editorRef.current?.innerHTML ?? ""),
    isEmpty: () => {
      const text = editorRef.current?.innerText ?? "";
      return text.trim() === "";
    },
    setHTML: (html: string) => {
      if (editorRef.current) {
        ...
        checkEmpty();
        // Sync html draft nếu đang ở tab html
        setHtmlDraft(html);
      }
    },
  }), [tab, htmlDraft, checkEmpty]);
  ...
  const formatHTML = (html: string) => {
    ...
    // Nếu nội dung không bắt đầu bằng thẻ HTML, bọc nó trong <p>
    let processedHtml = html;
    if (!html.trim().startsWith("<")) {
      processedHtml = `<p>${html}</p>`;
    }

    // Replace các thẻ div mặc định của contentEditable thành p cho chuẩn semantic
    processedHtml = processedHtml.replace(/<div>/gi, "<p>").replace(/<\/div>/gi, "</p>");

    // Đơn giản hoá việc format: thêm newline trước các thẻ block
    const blockTags = ['p', 'div', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'ul', 'ol', 'li', 'blockquote'];
    let formatted = processedHtml;
    
    blockTags.forEach(tag => {
      const regexOpen = new RegExp(`<${tag}(.*?)>`, 'gi');
      const regexClose = new RegExp(`</${tag}>`, 'gi');
      formatted = formatted.replace(regexOpen, `\n<${tag}$1>`);
      formatted = formatted.replace(regexClose, `</${tag}>\n`);
    });
  ...
  const switchToVisual = useCallback(() => {
    ...
    setTab("visual");
  }, [htmlDraft, checkEmpty]);

  const handleTabClick = (next: EditorTab) => {
    if (next === tab) return;
    if (next === "html") switchToHtml();
    else switchToVisual();
  };

  const tabBase =
    "flex items-center gap-1.5 px-3 py-1 text-xs font-semibold rounded-md transition-all select-none cursor-pointer";
  const tabActive = "bg-white text-slate-800 shadow-sm";
      ...
      <div className="flex items-center justify-between px-2 py-1.5 border-b border-slate-100 bg-slate-50">
        ...
        <div className="flex items-center gap-1 bg-slate-100 rounded-lg p-0.5">
          <button
            ...
            className={`${tabBase} ${tab === "visual" ? tabActive : tabInactive}`}
          >
            <Eye size={12} />
            Văn bản
          </button>
          <button
            ...
            className={`${tabBase} ${tab === "html" ? tabActive : tabInactive}`}
          >
            <Code2 size={12} />
            HTML
          </button>
        ...
        {/* Formatting toolbar — chỉ hiển thị ở tab visual */}
        {tab === "visual" && (
          <div className="flex items-center flex-wrap gap-0.5 overflow-x-auto">
            ...
            <ToolbarBtn onClick={() => exec("insertUnorderedList")} title="Danh sách không thứ tự">
              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M4 6h16M4 10h16M4 14h16M4 18h16" />
              </svg>
            </ToolbarBtn>
            <ToolbarBtn onClick={() => exec("insertOrderedList")} title="Danh sách có thứ tự">
              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M9 5h11M9 12h11M9 19h11M4 5v.01M4 12v.01M4 19v.01" />
              </svg>
            </ToolbarBtn>
            ...
            <ToolbarBtn onClick={() => exec("justifyCenter")} title="Căn giữa">
              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M4 6h16M8 12h8M6 18h12" />
              </svg>
            </ToolbarBtn>
        ...
        {tab === "html" && (
          <span className="text-[10px] font-bold text-amber-600 uppercase tracking-wider">
            HTML Source · Sẽ sync vào editor khi chuyển tab
          </span>
        )}
      </div>

      {/* ── Visual editor (contentEditable) ── */}
      <div className={`relative ${tab === "html" ? "hidden" : ""}`}>
        {isEmpty && (
      ...
      {tab === "html" && (
        <div>
          <textarea
            ...
            placeholder="Dán hoặc chỉnh sửa HTML tại đây..."
            rows={10}
            spellCheck={false}
            className="w-full px-3 py-3 text-sm font-mono bg-amber-50/30 focus:outline-none resize-y leading-relaxed"
            style={{ minHeight }}
          />
        </div>
      )}
